...
@ComponentFactory('BrythonPath-Factory')
@Provides(specifications=[IClientIndexPath.name])
@Requires("_log", IActivityLogger.name, spec_filter="'(name=main)'")
@Instantiate("BrythonPath")
class BrythonPath(IClientIndexPath):

    def __init__(self):
        super(IClientIndexPath, self).__init__();
        self._path =inspect.getmodule(self).__file__.replace("core{0}bundles{0}path{0}brython_path.py".format(os.path.sep), "")
        self._log =None
        self._id = "brython"
    ...
    def validate(self, context):
        _logger.info("BrythonPath validating")


        _logger.info("BrythonPath validated")

    @Invalidate
    def invalidate(self, context):
        _logger.info("BrythonPath invalidating")

        _logger.info("BrythonPath invalidated")
